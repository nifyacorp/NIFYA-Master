...
// Define request options - based on API explorer documentation
const options = {
  hostname: 'authentication-service-415554190254.us-central1.run.app',
  port: 443,
  path: '/api/auth/login',  // Correct endpoint from API explorer
  method: 'POST',
  headers: {
  ...
  res.on('end', () => {
    ...
    try {
      ...
      if (parsedData.accessToken) {
        fs.writeFileSync('auth_token.txt', parsedData.accessToken);
        console.log('Authentication successful! Access token saved to auth_token.txt');
...
==========================================
Status: SUCCESS
Access Token: ${parsedData.accessToken.substring(0, 10)}...
