...
import { fetchHttp } from '../services/fetcher/httpFetcher.js';
...
export async function analyzeText(req, res, next) {
  try {
    ...
    // Fetch content (only http type for MVP)
    const fetched = await fetchHttp(sourceConfig.url);

    // Analyze each prompt
    const analysisResults = await Promise.all(
      texts.map(prompt => analyzeGenericItems(fetched.items, prompt, req.id))
    );
    ...
    const response = {
      ...
      metadata: {
        ...
        total_items_processed: fetched.items.length,
        status: 'success'
      }
    };
